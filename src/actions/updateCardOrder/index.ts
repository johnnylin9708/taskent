...
const handler = async (data: InputType): Promise<ReturnType> => {
  ...
  try {
    const cardsPromises = await items.map(async (card) => {
      const cardsUpdated = await client.graphql({
      ...
      return cardsUpdated.data.updateCard;
    });

    cards = await Promise.all(cardsPromises);
